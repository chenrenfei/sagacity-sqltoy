...
public class SqlToyContext {
    ...
    public static SqlToyContext sqlToyContext = null;

    public static SqlToyContext getInstance() {
        return SqlToyContext.sqlToyContext;
    }

    private String connectionManagerName = null;

    public void setConnectionManagerName(String name) {
        this.connectionManagerName = name;
    }
    ...
    public String beanManagerName = null;

    public String getBeanManagerName() {
        return beanManagerName;
    }

    public void setBeanManagerName(String beanManagerName) {
    ...
    public void initialize() throws Exception {
        ...
        SqlToyContext.sqlToyContext = this;
...
//            }
            ...
            if (beanManager.containsBean(beanName)) {
                beanDefine = beanManager.getBean(beanName);
            } else if (beanName.indexOf(".") > 0) {
                beanDefine = beanManager.getBean(Class.forName(beanName));
            } else {
                return null;
            }
    ...
    public Object getBean(Object beanName) {
        try {
            ...
            if (beanName instanceof String) {
                ...
                return beanManager.getBean(beanName.toString());
            }
            return beanManager.getBean((Class) beanName);
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("从BeanManager中获取Bean:{} 错误!{}", e.getMessage());
        }
        return null;
    }
    ...
    public DataSource getDataSourceBean(String dataSourceName) {
        ...
        BeanManager beanManager = BeanManager.getBeanManager(beanManagerName);
        if (beanManager.containsBean(dataSourceName)) {
            return (DataSource) beanManager.getBean(dataSourceName);
        }
        return null;
    }
...
//    }
    ...
    public ShardingStrategy getShardingStrategy(String strategyName) {
        ...
        BeanManager beanManager = BeanManager.getBeanManager(beanManagerName);
        ShardingStrategy shardingStrategy = (ShardingStrategy) beanManager.getBean(strategyName);
...
//    }
    ...
    public DataSource obtainDataSource() {
        ...
        BeanManager beanManager=BeanManager.getBeanManager(beanManagerName);
        return obtainDataSource.getDataSource(beanManager, defaultDataSource);
    }
